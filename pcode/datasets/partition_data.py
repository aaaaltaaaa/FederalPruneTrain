...
def record_class_distribution(partitions, targets, print_fn):
    ...
    for idx, partition in enumerate(partitions):
        ...
        #     for i in range(10):
        #         data.append(0)
        #     for i,c in enumerate(unique_elements):
        #         print(i,c)
        #         data[c] = counts_elements[i]
        #     data = np.array(data)
        #     area.append(np.pi * (0.02 * data)** 2)
    ...
    #     plt.yticks(y_ticks)
    #     plt.scatter(xs, ys, s=area, alpha=1,color='orange')
    #     plt.xlabel('Client IDs',fontsize=20)
    #     plt.ylabel('Class labels',fontsize=20)
    #     plt.savefig('/home/wnpan/feddf/pictures/alpha01.pdf')
