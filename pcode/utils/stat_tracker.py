...
class LogitTracker():
    def __init__(self, unique_labels, device):
        self.unique_labels, self.device = unique_labels, device
        self.labels = [i for i in range(unique_labels)]
        self.label_counts = torch.ones(unique_labels).to(self.device) # avoid division by zero error
        self.logit_sums = torch.zeros((unique_labels,unique_labels) ).to(self.device)
    ...
    def update(self, logits, Y):
        ...
        logit_sums_ = torch.zeros((self.unique_labels, self.unique_labels)).to(self.device)
