...
class Master(object):
    ...
    def run(self):
        ...
        for comm_round in range(1, 1 + self.conf.n_comm_rounds):
            ...
            if self.conf.global_history:
                to_send_history = (self.global_perfomances[0] > performance) and (self.global_perfomances[1] > performance)
                if to_send_history:
    ...
    def _send_model_to_selected_clients(self, selected_client_ids,to_send_history = False):
        ...
        for worker_rank, selected_client_id in enumerate(selected_client_ids, start=1):
            ...
            dist.send(tensor=flatten_model.buffer, dst=worker_rank)
            dist.send(torch.ones(1),dst=worker_rank)
            self.conf.logger.log(
    ...
    def train_generator(self, selected_client_ids, label_weights, qualified_labels,flatten_local_models, epochs=10, verbose=True):
