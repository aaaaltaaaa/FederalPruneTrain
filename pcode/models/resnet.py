...
from pcode.models.lowrank_resnet import LowRankBasicBlock, LowRankBasicBlockConv1x1, LowRankBottleneckConv1x1, \
    HybridResNet, FullRankBasicBlock
...
class ResNet_imagenet(ResNetBase):
    def __init__(
        ...
        # define layers.
        self.inplanes = int(64 * scaler_rate)
        # self.conv1 = nn.Conv2d(
        #     in_channels=3,
        #     out_channels=64,
        #     kernel_size=7,
        ...
        self.bn1 = norm2d(group_norm_num_groups, planes=int(64 * scaler_rate), track_running_stats=track_stat)
        self.relu = nn.ReLU(inplace=True)
        ...
        if self.projection:
            self.projection_layer = nn.Sequential(
                nn.Linear(int(512 * block_fn.expansion * scaler_rate), int(512 * block_fn.expansion * scaler_rate)),
                nn.ReLU(),
                nn.Linear(int(512 * block_fn.expansion * scaler_rate), 256)
            )
            ...
            self.classifier = nn.Linear(
                in_features=int(512 * block_fn.expansion * scaler_rate), out_features=self.num_classes
            )
...
def resnet(conf, arch=None):
    rank_factor = 1
    if conf.low_rank:
        arch, rank_factor = arch.split('_')
        rank_factor = int(rank_factor)
    if conf.pruning:
        arch, scaler_rate = arch.split('_')
        scaler_rate = float(scaler_rate)
    ...
    if conf.low_rank:
        if rank_factor > 1:
            model_params = {
                18: {"block": LowRankBasicBlock, "layers": [2, 2, 2, 2]},
                34: {"block": LowRankBasicBlock, "layers": [3, 4, 6, 3]},
                50: {"block": LowRankBottleneckConv1x1, "layers": [3, 4, 6, 3]},
                101: {"block": LowRankBottleneckConv1x1, "layers": [3, 4, 23, 3]},
                152: {"block": LowRankBottleneckConv1x1, "layers": [3, 8, 36, 3]},
            }
            block = model_params[resnet_size]["block"]
            layers = model_params[resnet_size]["layers"]
            model = HybridResNet(block, FullRankBasicBlock, rank_factor=rank_factor, layers=layers,
                                 num_classes=decide_num_classes(dataset), track_running_stats=not conf.freeze_bn)
            # model = LowRankResNetConv1x1(block,rank_factor,layers,
            #                              num_classes=decide_num_classes(dataset))
        else:
        ...
        model = ResNet_imagenet(
            ...
            scaler_rate=scaler_rate,
            pruning=True,
            need_scaler=conf.freeze_bn_affine
        )
    else:
