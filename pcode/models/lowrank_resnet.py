...
class FullRankBasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1,
                 base_width=64, dilation=1, norm_layer=None, rank_factor=None, track_running_stats=True):
...
class LowRankBasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1,
                 base_width=64, dilation=1, norm_layer=None, rank_factor=1, track_running_stats=True):
...
class LowRankBasicBlockConv1x1(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1,
                 base_width=64, dilation=1, norm_layer=None, rank_factor=4, track_running_stats=True):
        ...
        # Both self.conv1 and self.downsample layers downsample the input when stride != 1
        # self.conv1_u = conv3x3(inplanes, int(planes / rank_factor), stride)
        self.conv1_u = conv3x1(inplanes, int(planes / rank_factor), stride)
        # self.bn1_u = norm_layer(int(planes / rank_factor))
        self.conv1_v = conv1x3(int(planes / rank_factor), planes)
        # self.conv1_v = conv1x3(int(planes / rank_factor), planes)
        # self.bn1_v = norm_layer(planes)
        self.bn1 = norm_layer(planes, track_running_stats=track_running_stats)
        self.relu = nn.ReLU(inplace=True)

        self.conv2_u = conv3x1(planes, int(planes / rank_factor))
        # self.conv2_u = conv3x1(planes, int(planes / rank_factor))
        # self.bn2_u = norm_layer(int(planes / rank_factor))
        self.conv2_v = conv1x3(int(planes / rank_factor), planes)
        # self.conv2_v = conv1x3(int(planes / rank_factor), planes)
...
class HybridResNet(nn.Module):
    def __init__(self, lowrank_block, fullrank_block, rank_factor, layers, num_classes=1000, zero_init_residual=False,
                 groups=1, width_per_group=64, replace_stride_with_dilation=None,
                 norm_layer=None, track_running_stats=True):
        ...
        # ================================================================================================================
        self.layer1 = self._make_layer(fullrank_block, 64, layers[0], track_running_stats=track_running_stats)
        self.layer2 = self._make_layer(fullrank_block, 128, layers[1], stride=2, rank_factor=rank_factor,
                                       dilate=replace_stride_with_dilation[0],
        ...
        self.layer3 = self._make_layer(lowrank_block, 256, layers[2], stride=2, rank_factor=rank_factor,
                                       dilate=replace_stride_with_dilation[1], track_running_stats=track_running_stats)
        ...
        self.layer4 = self._make_layer(lowrank_block, 512, layers[3], rank_factor=rank_factor, stride=2,
                                       dilate=replace_stride_with_dilation[2], track_running_stats=track_running_stats)
        # ================================================================================================================
    ...
    def _make_layer(self, block, planes, blocks, stride=1, rank_factor=None, dilate=False, track_running_stats=True):
        norm_layer = self._norm_layer
        downsample = None
        previous_dilation = self.dilation
        if dilate:
